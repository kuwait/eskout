-- Migration 086: Consolidated player profile RPC
-- Replaces 12-14 separate queries with a single function call
-- Returns all data needed to render a player profile page in one round-trip
...
BEGIN
  ...
  FROM (
    ...
    WHERE sh.player_id = p_player_id
  ) sub;
  ...
  -- 7. Training feedback with author names
  SELECT COALESCE(json_agg(sub ORDER BY sub.training_date DESC), '[]'::json) INTO v_feedback
  FROM (
  ...
  FROM (
    ...
    WHERE sp.player_id = p_player_id AND sp.club_id = p_club_id
  ) sub;
