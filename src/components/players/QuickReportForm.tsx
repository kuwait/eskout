...
function isDirty(form: FormState): boolean {
  return Object.values(form.ratings).some(r => r > 0)
    ...
    || form.opponentLevel !== ''
    || form.observedPositions.length > 0
...
export function QuickReportForm({ playerId, playerName, isGoalkeeper, onSuccess, onCancel, onDirtyChange, initialMatchContext }: QuickReportFormProps) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await submitQuickReport({
        ...
        observedPosition: form.observedPositions.length > 0 ? form.observedPositions.join(', ') : undefined,
  ...
  // Hide match context section when coming from a scouting game (already in header)
  const showMatchContext = !initialMatchContext?.gameId;

  return (
    <div className="space-y-4 overflow-hidden">

      {/* ── Posição Observada (multi-select) ── */}
      <div>
          ...
          {OBSERVED_POSITIONS.map(pos => (
            <button
              ...
              onClick={() => setForm(prev => ({
                ...prev,
                observedPositions: prev.observedPositions.includes(pos.code)
                  ? prev.observedPositions.filter(p => p !== pos.code)
                  : [...prev.observedPositions, pos.code],
              }))}
              className={cn(
                'rounded-md px-2.5 py-1.5 text-xs font-medium transition',
                form.observedPositions.includes(pos.code)
      ...
      {/* ── Físico (identical to training feedback) ── */}
      <div className="rounded-xl border border-l-[3px] border-l-cyan-400 bg-neutral-50/50 p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
          <ScaleSelector label="Estatura" info="Alto = acima da média · Normal = na média · Baixo = abaixo" options={[{ value: 'alto', label: 'Alto' }, { value: 'normal', label: 'Normal' }, { value: 'baixo', label: 'Baixo' }]}
            value={form.heightScale} onChange={(v) => setForm(prev => ({ ...prev, heightScale: v as HeightScale | '' }))} />
          <ScaleSelector label="Corpo" info="Ecto = magro/longilíneo · Meso = atlético · Endo = robusto" options={[{ value: 'ectomorfo', label: 'Ecto' }, { value: 'mesomorfo', label: 'Meso' }, { value: 'endomorfo', label: 'Endo' }]}
            value={form.buildScale} onChange={(v) => setForm(prev => ({ ...prev, buildScale: v as BuildScale | '' }))} />
          <ScaleSelector label="Velocidade" info="Rápido = destaca-se · Normal = na média · Lento = abaixo" options={[{ value: 'rapido', label: 'Rápido' }, { value: 'normal', label: 'Normal' }, { value: 'lento', label: 'Lento' }]}
            value={form.speedScale} onChange={(v) => setForm(prev => ({ ...prev, speedScale: v as SpeedScale | '' }))} />
          <ScaleSelector label="Intensidade" info="Intenso = esforço máximo · Pouco = baixa energia" options={[{ value: 'intenso', label: 'Intenso' }, { value: 'pouco_intenso', label: 'Pouco intenso' }]}
            value={form.intensityScale} onChange={(v) => setForm(prev => ({ ...prev, intensityScale: v as IntensityScale | '' }))} />
        </div>
        <ScaleSelector label="Maturação" info="Nada = pré-pubertário · Início = início do pico de crescimento · Maturado = pico atingido · Super = muito avançado para a idade" options={[{ value: 'nada_maturado', label: 'Nada' }, { value: 'a_iniciar', label: 'Início' }, { value: 'maturado', label: 'Maturado' }, { value: 'super_maturado', label: 'Super' }]}
          value={form.maturationScale} onChange={(v) => setForm(prev => ({ ...prev, maturationScale: v as MaturationScale | '' }))} />
      </div>

      {/* ── Dimension cards (Técnica, Tática, Físico, Mentalidade, Potencial) ── */}
      {DIMENSIONS.map(dim => (
        <DimensionCard
          key={dim.key}
          dimension={dim}
      ...
      {/* ── Contexto da Observação (same card style as Físico) ── */}
      <div className="rounded-xl border border-l-[3px] border-l-purple-400 bg-neutral-50/50 p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2">
          <ScaleSelector label="Pé observado" options={FOOT_OPTIONS.map(o => ({ value: o.value, label: o.label }))}
            value={form.observedFoot} onChange={(v) => setForm(prev => ({ ...prev, observedFoot: v as QuickReportFoot | '' }))} />
          <ScaleSelector label="Titular / Suplente" options={STARTER_OPTIONS.map(o => ({ value: o.value, label: o.label }))}
            value={form.starter} onChange={(v) => setForm(prev => ({ ...prev, starter: v as QuickReportStarter | '', subMinute: v === 'Titular' ? '' : prev.subMinute }))} />
          <ScaleSelector label="Destaque" options={STANDOUT_OPTIONS.map(o => ({ value: o.value, label: o.label }))}
            value={form.standoutLevel} onChange={(v) => setForm(prev => ({ ...prev, standoutLevel: v as QuickReportStandout | '' }))} />
          <ScaleSelector label="Adversário" options={OPPONENT_LEVEL_OPTIONS.map(o => ({ value: o.value, label: o.label }))}
            value={form.opponentLevel} onChange={(v) => setForm(prev => ({ ...prev, opponentLevel: v as QuickReportOpponentLevel | '' }))} />
        </div>
        {/* Minutes row */}
        <div className="grid grid-cols-2 gap-x-3">
          <div>
            <p className="mb-1 text-[10px] font-medium text-neutral-500">Min. observados</p>
            <Input type="number" inputMode="numeric" min={1} max={120} value={form.minutesObserved}
              onChange={e => setForm(prev => ({ ...prev, minutesObserved: e.target.value }))} placeholder="Ex: 70" className="h-8 text-xs" />
          </div>
          {form.starter === 'Suplente' && (
            <div>
              <p className="mb-1 text-[10px] font-medium text-neutral-500">Min. entrada</p>
              <Input type="number" inputMode="numeric" min={1} max={120} value={form.subMinute}
                onChange={e => setForm(prev => ({ ...prev, subMinute: e.target.value }))} placeholder="Min" className="h-8 text-xs" />
            </div>
          )}
        </div>
        {/* Condições */}
        <div>
          <p className="mb-1 text-[10px] font-medium text-neutral-500">Condições</p>
      ...
      {/* ── Contexto do Jogo (hidden when coming from scouting game — already in header) ── */}
      {showMatchContext && (
        <div className="rounded-xl border border-l-[3px] border-l-neutral-300 bg-neutral-50/50 p-3 space-y-3">
          <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-500">Contexto do Jogo</p>
          <div className="grid grid-cols-2 gap-x-3 gap-y-2">
            <div>
              <p className="mb-1 text-[10px] font-medium text-neutral-500">Competição</p>
              <Input value={form.competition} onChange={e => setForm(prev => ({ ...prev, competition: e.target.value }))} placeholder="Campeonato Distrital" className="h-8 text-xs" />
            </div>
            <div>
              <p className="mb-1 text-[10px] font-medium text-neutral-500">Data</p>
              <Input type="date" value={form.matchDate} onChange={e => setForm(prev => ({ ...prev, matchDate: e.target.value }))} className="h-8 text-xs" />
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] font-medium text-neutral-500">Jogo</p>
            <Input value={form.opponent} onChange={e => setForm(prev => ({ ...prev, opponent: e.target.value }))} placeholder="Equipa A vs Equipa B" className="h-8 text-xs" />
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <Textarea
          ...
          className="resize-none text-xs placeholder:text-xs placeholder:text-muted-foreground/40"
          rows={3}
        />
      </div>

      {/* ── Global + Recomendação (before submit) ── */}
        ...
        <div className="flex h-10 gap-0.5 rounded-xl overflow-hidden">
          {[1, 2, 3, 4, 5].map(n => (
            <button
              key={n}
              type="button"
              onClick={() => setOverall(form.ratingOverall === n ? 0 : n)}
              className={cn(
      ...
      {/* ── Submit ── */}
      <div className="sticky bottom-0 bg-gradient-to-t from-white via-white to-transparent pb-[env(safe-area-inset-bottom)] pt-2">
        {/* Missing fields hint */}
        {!allRated && (() => {
          const missing: string[] = [];
          for (const dim of DIMENSIONS) {
            if (form.ratings[dim.key] === 0) missing.push(dim.label);
          }
          if (form.ratingOverall === 0) missing.push('Global');
          if (form.recommendation === '') missing.push('Recomendação');
          return missing.length > 0 ? (
            <div className="mb-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-center dark:border-red-800 dark:bg-red-950/20">
              <p className="text-[11px] text-red-600 dark:text-red-400">
                <span className="font-semibold">Falta avaliar:</span>{' '}
                {missing.map((m, i) => (
                  <span key={m}>
                    {i > 0 && <span className="text-red-300 dark:text-red-700"> · </span>}
                    {m}
                  </span>
                ))}
              </p>
...
/** Physical scale selector — identical to ScaleBlock in TrainingFeedback.tsx */
function ScaleSelector({ label, options, value, onChange, info }: {
  label: string;
