        ...
        <div>
          {allPlayers.length === 0 ? (
            ...
            <div className="space-y-2">
              {allPlayers.map((p) => (
                <Link key={p.id} href={`/jogadores/${p.id}`} className="flex items-center gap-3 rounded-lg border bg-white p-3 hover:bg-neutral-50 transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium truncate">{p.name}</p>
                      <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${ROLE_COLORS[p.createdByRole] ?? 'bg-neutral-100 text-neutral-600'}`}>
