...
import { Eye, EyeOff, Plus, Search, X, StickyNote, Users } from 'lucide-react';
    ...
    <div className="p-4 lg:p-6">
      ...
      {myList.length > 0 && (
        <div className="space-y-2">
          {myList.map((entry) => (
            <div key={entry.id} className="rounded-lg border bg-white transition-colors hover:bg-neutral-50">
              <div className="flex items-center gap-3 p-3">
                <Link href={`/jogadores/${entry.playerId}`} className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium truncate">{entry.playerName}</p>
                    {entry.playerPosition && (
                      <span className="shrink-0 rounded bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium">
                        {POSITION_LABELS[entry.playerPosition] ?? entry.playerPosition}
                      </span>
                    )}
                    {entry.playerNationality && (
                      <span className="text-xs">{getNationalityFlag(entry.playerNationality)}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <ClubBadge club={entry.playerClub} logoUrl={entry.playerClubLogoUrl} size="xs" className="text-muted-foreground" />
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    ...
                    className={`h-7 w-7 inline-flex items-center justify-center rounded-md transition-colors ${
                      entry.note ? 'text-amber-500 hover:bg-amber-50' : 'text-neutral-300 hover:text-neutral-500 hover:bg-neutral-50'
                    }`}
                    title={entry.note ?? 'Adicionar nota'}
                  >
                  ...
                  <button
                    ...
                    className="h-7 w-7 inline-flex items-center justify-center rounded-md text-neutral-300 transition-colors hover:text-red-500 hover:bg-red-50 disabled:opacity-50"
                    title="Remover"
                  >
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-white shadow-xl animate-in slide-in-from-right duration-200">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{entries.length} entradas de todos os utilizadores</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {entries.length === 0 ? (
            <p className="text-center text-sm text-muted-foreground py-12">Nenhuma lista de observação</p>
          ) : (
            <div className="space-y-5">
              {[...grouped.entries()].map(([ownerName, ownerEntries]) => (
                <div key={ownerName}>
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">
                    {ownerName} ({ownerEntries.length})
                  </p>
                  <div className="rounded-lg overflow-hidden">
                    {ownerEntries.map((e, idx) => (
                      <Link
                        key={e.id}
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground truncate">
                            {e.playerClub} · {formatDate(e.createdAt)}
                          </p>
                          {e.note && (
