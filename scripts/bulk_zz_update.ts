...
const MODE = (process.env.MODE || 'all') as 'find' | 'scrape' | 'all';
...
/* ───────────── Timing — aggressive anti-blocking ───────────── */

// Between individual ZZ page fetches
const PAGE_DELAY_MIN = 5_000;
const PAGE_DELAY_MAX = 12_000;

// Between autocomplete search requests (lighter endpoint, can be faster)
const SEARCH_DELAY_MIN = 3_000;
const SEARCH_DELAY_MAX = 7_000;
...
// Between batches
const BATCH_PAUSE_MIN = 30_000;
const BATCH_PAUSE_MAX = 90_000;

// Backoff on block detection
const BLOCKED_BACKOFF_MS = 300_000; // 5 minutes
...
/** Find the ZZ link for a player via autocomplete search + DOB verification.
...
async function findZzLink(
  ...
  for (const candidate of shortlisted) {
    ...
    // DOB exists but doesn't match → wrong person, skip
  }
