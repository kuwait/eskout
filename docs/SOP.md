...
15. **Recruitment pipeline statuses** (current, after migration 090): `por_tratar`, `em_contacto`, `vir_treinar`, `reuniao_marcada`, `a_decidir`, `em_standby`, `confirmado`, `assinou`, `rejeitado`. The old `a_observar` status was migrated to the `player_lists` system (via `user_observation_list` → `player_lists` + `player_list_items`). The `a_decidir` status is split into two sub-sections via `decision_side` (`'club'` or `'player'`) — see migration 058. The `em_standby` status requires a mandatory `standby_reason` text — see migration 090.
16. **Department opinions** (10 values): `1ª Escolha`, `2ª Escolha`, `Acompanhar`, `Por Observar`, `Urgente Observar`, `Sem interesse`, `Potencial`, `Ver em treino`, `Stand-by`, `Assinar`. Stored as `text[]` on players — no DB CHECK constraint, validated app-side only.
